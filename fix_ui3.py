...
with open('old_health.jsx', 'r', encoding='utf-8') as f:
    old_health = f.read()
...
# Add states for the new rule form and the list of rules
...
config_jsx = config_jsx.replace(
    ...
    '<button type="button" onClick={() => { if(newRule.name && newRule.contact) { setRules([...rules, { id: Date.now(), ...newRule }]); setNewRule({ name: \'\', contact: \'\', loc: \'All Locations\', type: \'All Violations\' }); showToast(\'Alert Rule Added!\'); } }} style={{ padding: \'8px 16px\', background: \'var(--accent-blue)\', color: \'white\', border: \'none\', borderRadius: \'6px\', cursor: \'pointer\' }}>+ Add Recipient</button>'
)

# Replace static table body with dynamic
static_tbody = '''<tbody>
                <tr>
...
dynamic_tbody = '''<tbody>
                {rules.map(rule => (
                  <tr key={rule.id}>
                    ...
                    <td style={{ padding: '12px', borderBottom: '1px solid #f1f5f9', color: '#ef4444', cursor: 'pointer' }} onClick={() => { setRules(rules.filter(r => r.id !== rule.id)); showToast('Rule Removed', 'error'); }}>Remove</td>
                  </tr>
                ))}
              </tbody>'''
...
# Fix broken thermal images and empty out the alt tags
...
# Inject the Global Site Health Widget into the Overview Camera Matrix location
...
# We need to replace the entire old split-card-left block in Dashboard.jsx.
# Since it's multiline, we'll use regex or string slicing.
import re
