...
import re
# Regex to find EXACTLY the block from <div className="data-grid camera-matrix" to the FIRST </div> that has no indentation (or just use string finding)
# Wait, let's just find the exact string to replace!

exact_original = '''<div className="data-grid camera-matrix" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px', marginTop: 0 }}>
...
if exact_original in code:
    code = code.replace(exact_original, '<div style={{ marginTop: "0px" }}><SiteHealthHeatmap /></div>')
else:
    print("COULD NOT FIND EXACT ORIGINAL STRING")
