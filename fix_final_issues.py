...
# 1. Create AlertRulesView.jsx
alert_rules_code = '''import React, { useState } from 'react';
...
with open('src/components/AlertRulesView.jsx', 'w', encoding='utf-8') as f:
    f.write(alert_rules_code)

# 2. Update Dashboard.jsx
with open('src/components/Dashboard.jsx', 'r', encoding='utf-8') as f:
    dashboard = f.read()

dashboard = dashboard.replace("import ReportsView from './ReportsView';", "import ReportsView from './ReportsView';\nimport AlertRulesView from './AlertRulesView';")
dashboard = dashboard.replace("{activeSolution === 'Reports' && <ReportsView />}", "{activeSolution === 'Reports' && <ReportsView />}\n          {activeSolution === 'Alert Rules' && <AlertRulesView />}")
# Change the horizontal grid for thermal sensors
dashboard = dashboard.replace('className="data-grid"', 'style={{ display: \'grid\', gridTemplateColumns: \'repeat(auto-fit, minmax(300px, 1fr))\', gap: \'24px\' }}')

with open('src/components/Dashboard.jsx', 'w', encoding='utf-8') as f:
    f.write(dashboard)

# 3. Clean up ConfigurationView.jsx (Optional, remove the Alert Rules tab from there to avoid duplication)
with open('src/components/ConfigurationView.jsx', 'r', encoding='utf-8') as f:
    config = f.read()
config = config.replace("'Sites', 'Cameras', 'Use Cases', 'Alert Rules'", "'Sites', 'Cameras', 'Use Cases'")
# We can just leave the code block {activeTab === 'Alert Rules' && ...} in there because it will never be triggered now,
# but to be clean, let's let it be. 
